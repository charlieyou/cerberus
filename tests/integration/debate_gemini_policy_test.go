...
func TestDebateGeminiPolicy(t *testing.T) {
	...
	t.Run("two gemini panel", func(t *testing.T) {
		...
		assertGeminiMockInvocations(t, runRoot, []string{"gemini#1", "gemini#2"}, 4)
	})
}
...
func assertGeminiMockInvocations(t *testing.T, runRoot string, instanceIDs []string, want int) {
	t.Helper()
	...
	if count != want {
		t.Fatalf("gemini policy-checked invocations = %d, want %d", count, want)
	}
}
