...
**Result**: a thin set of additions to cerberus that turns a plan → tasks → autonomous implementer/reviewer loop on top of Claude's **agent-team** primitive (`TeamCreate` + `Agent({team_name, name, …})`), gated by `TaskCompleted`-driven multi-model review, and capped by an automatic `/cerberus:verify-epic` pass at the end of the run.
...
User: /cerberus:run-team                        (NEW command → kicks off the team)
   ...
   └── Lead Claude (team lead)
         ...
         └── Scheduling loop (strictly serial in initial cut):
               ...
               │          prompt: "<task spec + claim Claude task X via TaskUpdate(owner=…)>"})
               │     │
               │     ├── teammate reads task spec, implements, commits with `Cerberus-Task: T###` trailer
               │     ├── teammate calls TaskUpdate(status: "completed")    ← triggers TaskCompleted hook
               │     │     │
               ...
               │     │                 │   user-confirmed verification gate, spawns review,
               │     │                 │   polls wait
               ...
               │     │                 │   feedback, fixes, retries TaskUpdate
         ...
         │
         ├── Phase 4: when all cerberus tasks are completed (and no failures):
         │     /cerberus:verify-epic against the plan's acceptance criteria → final epic-level gate.
         │
         └── Final report (passed / failed / skipped + verify-epic verdict).
```
...
**TaskCompleted firing semantics (clarified per docs).** `TaskCompleted` fires only when a task is being marked completed — i.e., on an explicit `TaskUpdate(status: "completed")` call (https://code.claude.com/docs/en/hooks: "When a task is being marked as completed"). It does NOT fire when a teammate goes idle or finishes a turn — that's `TeammateIdle`. So:
- The hook never fires "spuriously" on the implementer's escalation path. After a final-round FAIL, the implementer's prompt rule says to NOT retry `TaskUpdate(completed)`; if that rule is followed, the hook does not fire again on this task.
- The lead detects an idle teammate via the (separate) `TeammateIdle` event + TaskList state inspection. The lead does NOT depend on `TaskCompleted` for idle detection.
- The `abandoned` and `needs-human` outcomes (Phase 2) classify cases where the teammate went idle without completing the task; both depend on `TeammateIdle` + TaskList state, not on `TaskCompleted`.
...
- **Phase 1: Load** — resolve `--from-tasks`: if explicit, use it; otherwise auto-detect by finding the most recent `*-plan.md` in `docs/` or `~/.claude/plans/` (mirroring how `create-tasks` resolves `--from-plan`) and looking for a `*-team-tasks.md` adjacent to it (matching the prefix-based naming above). Error if zero or multiple candidates. Parse YAML frontmatter + each task's `meta` block, build dependency graph. Resolve `--max-review-rounds` (default 3) and `--skip-verify` (default false, applies only to the final epic verifier, not to the per-task project verification gate).
- **Phase 1.5: Project verification discovery + user confirmation (hard gate before team start)** — before `TeamCreate`, `TaskCreate`, or any implementer `Agent` call, the lead discovers the project-specific verification commands and asks the user to confirm them.
  ...
  - Present the exact proposed commands to the user, grouped by purpose (`lint/static`, `build/typecheck`, `tests`), explain why each was chosen, and call out any missing category. Then ask the user to confirm or edit the command list.
  - **Do not start the team until the user explicitly confirms the verification gate.** "Start the team" means no `TeamCreate`, no `TaskCreate`, and no implementer `Agent` calls. Computing candidates and asking the user is safe; spawning teammates is not.
  - If the user explicitly confirms that no automated verification should run, write an explicit no-op script that prints that decision. Do not silently omit verification state; the hook fails closed if the confirmed script is absent.
  - Persist the confirmed commands as `${TMPDIR:-/tmp}/cerberus-task-completed-hook/<team_hash>/verify.sh`; every per-task `state.json` stores `verify_script_path` pointing to that script. The `TaskCompleted` hook runs this script after commit/trailer guards and before `spawn-code-review`.
- **Phase 2: Team setup and scheduling** — **strictly serial in the initial cut** (max 1 implementer at a time). Reasons: (a) all implementers share the same working tree, so concurrent edits would conflict; (b) review-gate state is keyed per top-level session — concurrent hook invocations would clobber each other (see open question 2). With strict serialization, dependent tasks (e.g., T002 depends on T001) automatically see prior commits in the working tree because T001 has finished and committed before T002 is spawned.
  - **Team setup (once, at start)**:
    ...
    3. Write the user-confirmed verification script to `${TMPDIR:-/tmp}/cerberus-task-completed-hook/${team_hash}/verify.sh`, with `set -euo pipefail` followed by the confirmed commands. This happens after confirmation but before `TeamCreate`, so the hook can fail closed if the script is missing.
  ...
  - **Lead's hard rules** (these go in the lead prompt verbatim):
    ...
    - The lead may call `TaskUpdate(owner: …)` and `TaskUpdate(status: "deleted")` (for canceling a task that the lead decides to abort). The lead must NOT call any `TaskUpdate` that moves status to `completed`.
  ...
  - **Scheduling loop**:
    ...
    - Pick one ready task (lowest ID first). Before spawning:
      1. **Create the per-task state dir**: `mkdir -p "${TMPDIR:-/tmp}/cerberus-task-completed-hook/${team_hash}/${task_id}"`. The `${team_hash}/` parent was created in team-setup, but the per-task subdirectory is not, so without this `mkdir` the subsequent `state.json` / `task-context.md` / `untracked_baseline.txt` writes would fail with `ENOENT` and the hook would fall into the `_unknown_<pid>` recovery path on the very first task.
      2. `git rev-parse HEAD` → write to per-task state file at `${TMPDIR:-/tmp}/cerberus-task-completed-hook/<team_hash>/<task_id>/state.json` containing `{ "task_id": "T001", "claude_task_id": "<id>", "team_hash": "<hash>", "baseline_sha": "<sha>", "round": 0, "max_rounds": <resolved>, "task_state_dir": "...", "task_context_path": "${state_dir}/task-context.md", "verify_script_path": "${state_root}/verify.sh" }`. The `<team_hash>` namespace ensures that retries of the same plan (different invocation → different team_hash, by construction from timestamp + random) and reruns of the same task ID across plans never inherit stale `exhausted` or `last_error` markers from earlier runs.
      3. Write a task-context file at `${state_dir}/task-context.md` containing the task spec body, files list, dependencies, acceptance criteria from `team-tasks.md`, and the user-confirmed verification script path (the same task content the implementer is given, plus the gate the hook will enforce). The hook will pass this to `spawn-code-review` via `--context-file` so reviewers see task background, not just the diff; the hook also injects the verification pass summary as `REVIEW_GATE_AUTHOR_CONTEXT` so reviewers can rely on the confirmed lint/build/test evidence.
      ...
      5. `Agent({ team_name: "cerberus-impl-<hash>", name: "impl-T001", subagent_type: "implementer", description: "T001 implement <subject>", prompt: <task spec + CERBERUS_TASK_ID=T001 + STATE_DIR=${TMPDIR:-/tmp}/cerberus-task-completed-hook/<team_hash>/T001 + VERIFY_SCRIPT=${TMPDIR:-/tmp}/cerberus-task-completed-hook/<team_hash>/verify.sh + claim-instruction "claim Claude task <id> via TaskUpdate(owner: 'impl-T001')" + commit-trailer instruction + completion-intent-marker instruction "before each TaskUpdate(status: 'completed'), Bash: touch \"$STATE_DIR/completion_intent\""> })`. Each task gets a fresh `name`, so each task spawns a **fresh teammate with no prior context**. Within this teammate's lifetime, the implementer/reviewer retry loop is in-context (the same teammate sees verification failures and review findings on stderr and retries). The `STATE_DIR` injection gives the implementer a concrete path to `touch` — without it, the implementer would have to compute the path itself from team_hash + task_id, which is fragile.
    - **Wait for outcome.** The lead receives `TeammateIdle` notifications automatically when the teammate stops working. After each idle, the lead inspects:
      - The Claude TaskList task's status (use `TaskGet`): `completed`, `in_progress`, etc.
      ...
      - **success** — Claude task status is `completed`, no `exhausted` marker, no `last_error` marker, both `${state_dir}/verified_pass` and `${state_dir}/reviewed_pass` markers are present, AND `${state_dir}/verified_head` plus `${state_dir}/reviewed_head` both equal `git rev-parse HEAD` (positive evidence that the hook ran the confirmed verification gate and code review against the current code). Without both markers and matching head files, the success classification is INVALID and the lead must downgrade to `unverified-failure` (see below). Provisionally classify as success (do NOT yet mark T### done or delete state dir). Run the post-task clean-tree gate (see below). If both gate checks pass: mark cerberus T### done in the lead's local tracking, **then** delete the per-task state directory at `${TMPDIR:-/tmp}/cerberus-task-completed-hook/<team_hash>/<task_id>/`, recompute ready set. If either gate check fails: downgrade to unverified-failure (per the existing rule), retain state dir, stop scheduling.
      - **needs-human** — classify as needs-human if EITHER (a) the teammate sent a `STATUS: NEEDS_HUMAN T###` message AND the Claude task status is still `in_progress`, OR (b) the `exhausted` marker is present AND the Claude task status is still `in_progress` (regardless of whether the NEEDS_HUMAN message was received). Rationale: the marker is authoritative — the hook wrote it because review rounds were exhausted, so the task cannot succeed even if the implementer didn't escalate cleanly. Mark failed; retain state dir; stop scheduling (see Phase 3).
      - **unverified-failure** — any of the following: (a) Claude task status is `completed` AND `exhausted` marker present (the implementer ignored the post-exhaustion rule and somehow got `TaskUpdate(completed)` past the hook — should be impossible if the hook is correct, but the lead defends against it); or (b) Claude task status is `completed` AND no `exhausted` marker AND no `last_error` marker AND either `verified_pass`/`reviewed_pass` is missing or `verified_head`/`reviewed_head` does not equal current `HEAD` — "task marked completed but missing positive verification/review evidence for the current HEAD — hook did not run, did not run the confirmed project verification gate, did not run review to PASS, or HEAD changed after the gate. Possible causes: TaskCompleted input field mismatch (the prefix filter received empty subject), hook script error, missing `verify_script_path`, runtime hook-firing variance, or external worktree mutation." Mark failed; retain state dir; stop scheduling.
      - **infra-failure** — `last_error` marker present (review-gate ERROR / no_reviewers / PENDING / timeout, or the missing-state recovery path where the hook could not resolve the `<team_hash>/<task_id>` state dir and falls back to a per-PID `${TMPDIR:-/tmp}/cerberus-task-completed-hook/_unknown_<pid>/last_error`). Claude task status is typically `in_progress` (hook blocked the completion). Mark failed; retain state dir; surface the raw wait JSON from `last_error` in the final report; stop scheduling. (The fallback is per-PID rather than a shared `_unknown` so concurrent or sequential runs that hit the missing-state path do not clobber each other's `last_error`.)
      - **abandoned** — Claude task status is `in_progress`, the teammate has gone idle (TeammateIdle received), AND none of the above markers/messages apply (no `exhausted`, no `last_error`, no `NEEDS_HUMAN` message, no `completed` status). The teammate left work unfinished without escalating — the implementer ignored its prompt rule (e.g., confused itself, the hook errored in a way that bypassed marker writing, or the teammate's first turn ended without doing anything). Mark failed; retain state dir; stop scheduling. This is a model-behavior failure, not infra; the lead should surface a clear message in the final report distinguishing it from `needs-human` (e.g., "T### abandoned: implementer went idle without completing the task or escalating; this is a model-behavior bug, not an infra failure").
    ...
    - **Cleanly release the teammate after a successful task**: after a success classification, the teammate has already gone idle. The lead does not need to message it again. Optionally, the lead can `SendMessage({to: "impl-T001", message: {type: "shutdown_request", reason: "task complete"}})` to terminate the teammate process; otherwise the teammate stays idle until the team is deleted in Phase 4. The fresh-teammate-per-task rule means we never reuse `impl-T001` for a later task — its name is task-bound by construction.
...
- **Phase 4: Epic verification + report**:
  - **Auto-verify-epic gate (NEW)**: if all cerberus tasks completed successfully (no failures) AND `--skip-verify` is not set AND the team-tasks.md frontmatter has a `plan` field that resolves to a real file, the lead invokes the existing `/cerberus:verify-epic` command against the plan as the final acceptance-criteria gate. Implementation: shell out to `${CLAUDE_PLUGIN_ROOT}/bin/review-gate spawn-epic-verify`, exporting `REVIEW_GATE_SESSION_KEY` on the spawn so the matching `wait --session-key` can find the gate state (`wait` looks up gates via `find_state_by_session_key`, which only matches when the spawn was invoked with that env var set). `spawn-epic-verify` also `die`s if `CLAUDE_SESSION_ID` is unset, so it must be present in the env — but the lead is itself a Claude session, so its Bash tool already has `CLAUDE_SESSION_ID` and `REVIEW_GATE_TRANSCRIPT_PATH` (or `CLAUDE_TRANSCRIPT_PATH`) automatically exported; only `REVIEW_GATE_SESSION_KEY` is per-invocation and must be set explicitly inline. `spawn-epic-verify` reads `CLAUDE_SESSION_ID` from the lead's env, and `wait` reads it the same way. Note: `spawn-epic-verify` does NOT accept a `--context-file` flag — the verifier reads the plan/spec directly. Use this exact command shape:
    ```
    ...
    ```
    The verdict is included in the final report. A FAIL or NEEDS_WORK-with-P0/P1 verdict is surfaced loudly (the user sees that implementation passed code review but the epic-level acceptance criteria are not satisfied — typically signals a missed AC or a wiring gap that per-task review couldn't catch). The verify-epic call uses cerberus's existing prompt at `prompts/reviewers/epic-verify.md` — no new prompt is added.
  - **Final report**: table of cerberus tasks (passed / failed / skipped) with commit hashes and review summaries; the user-confirmed per-task verification gate commands; for failed tasks, name the outcome category (`needs-human`, `unverified-failure`, `infra-failure`, or `abandoned`) so the user can distinguish model-behavior failures (`abandoned`) from infra failures and from clean human-escalations; the verify-epic verdict + any findings; recovery instructions for any failures. **Retry guidance**: include a note that re-running `/cerberus:run-team` after fixing the root cause is safe — each invocation gets a fresh `team_hash` (timestamp + random) and therefore a fresh state directory, so users do **not** need to manually wipe `${TMPDIR:-/tmp}/cerberus-task-completed-hook/` between attempts. Stale state from a prior failed run remains on disk for debugging but cannot affect the new run.
  - **Cleanup policy**:
    - Delete per-task state directories under `${TMPDIR:-/tmp}/cerberus-task-completed-hook/<team_hash>/` only for tasks that **completed successfully**. Failed tasks' state dirs are retained for debugging at `${TMPDIR:-/tmp}/cerberus-task-completed-hook/<team_hash>/<task_id>/`. To wipe all state for this run after debugging, delete the entire `<team_hash>` subdirectory; this is safe because the team_hash is unique to this invocation (constructed from path + second-precision timestamp + 8 hex chars from `/dev/urandom`) and cannot collide with other plans, concurrent runs, or same-day retries.
...
- **Workflow**:
  ...
  2. Claim the task: `TaskUpdate(taskId: "<claude-task-id>", owner: "<your-name>", status: "in_progress")`. (You may also be pre-claimed by the lead.)
  ...
  5. **Write completion-intent marker, then signal completion**: the lead injects an env var `STATE_DIR` in your spawn prompt (e.g., `STATE_DIR=/tmp/cerberus-task-completed-hook/<team_hash>/T001`). Immediately before calling `TaskUpdate(taskId: "<claude-task-id>", status: "completed")`, run `Bash: touch "$STATE_DIR/completion_intent"`. The hook checks for this marker as one of its first gates after ownership is established: if absent, the hook blocks completion with stderr telling you to write the marker and retry. The hook deletes the marker on every fire (success or failure) so each subsequent `TaskUpdate(completed)` requires a fresh `touch`.

     Then call `TaskUpdate(taskId: "<claude-task-id>", status: "completed")`. This fires the `TaskCompleted` hook, which runs review.
  6. The hook will either:
     ...
     - **Block the completion (exit 2 with stderr feedback)** — review found blocking issues. You will see the findings in your context (the stderr is injected as feedback). Fix the issues, recommit (new commit, also tagged `Cerberus-Task: T###`), and retry `TaskUpdate(status: "completed")`.
...
Add a `TaskCompleted` entry (no matcher; the hook script self-gates on `task_name` prefix and metadata):
...
The hook is registered globally on this plugin, but the script's first action is to bail out (exit 0) on any TaskCompleted that isn't a cerberus implementer task. So unrelated task lists (the user's own todos, other plugins' lists) are unaffected.
...
3.4. **Completion-intent gate.** Read `${state_dir}/completion_intent`; if absent, block completion with exit 2 and stderr telling the implementer to run `touch "${state_dir}/completion_intent"` immediately before retrying `TaskUpdate(status:'completed')`. For a `[CERBERUS-IMPL/<team_hash>]` task, ownership has already been established and `TaskCompleted` means the task is being marked complete, so missing intent must fail closed. If present, **delete it** before proceeding (so each subsequent `TaskUpdate(completed)` requires a fresh `touch` from the implementer):
   ```
   if [[ ! -f "${state_dir}/completion_intent" ]]; then
     printf 'CERBERUS VERIFY: missing completion_intent marker for %s. Run: touch "%s/completion_intent", then retry TaskUpdate(status:'"'"'completed'"'"').\n' "$task_id" "$state_dir" >&2
     exit 2
   fi
   rm -f "${state_dir}/completion_intent"
   ```
   This is a fail-closed guard. The implementer's prompt requires the `touch` immediately before `TaskUpdate(completed)`, and missing it should not allow an owned Cerberus task to complete without verification and review.

3.5. **Idempotent on prior PASS (defensive).** If a previous `TaskCompleted` invocation for this same task already passed both the confirmed verification gate and review, this is a re-fire. Let it through without re-running verification or review:
   ```
   ...
   ```
   **Defensive idempotency.** The plan reads the official docs (https://code.claude.com/docs/en/hooks) as: TaskCompleted fires only on explicit `TaskUpdate(status: "completed")`. But for safety against runtime variance, future docs changes, or retry storms, the hook short-circuits if both positive-evidence markers already exist for the current `HEAD`. Without this guard, a re-fire after a successful first pass would rerun verification and burn an additional review round (and possibly fail spuriously). With the guard, the second fire is a no-op only when the exact current code was already verified and reviewed. This pairs with the exhaustion short-circuit in step 4: between the positive-evidence check here and the `exhausted` check there, the hook is robust to any TaskCompleted re-fire after a terminal outcome (PASS or exhaustion-driven escalation).

4. **Short-circuit on max rounds (exhaustion).** Read `round` and `max_rounds` from the state file. If `round >= max_rounds`, write a marker file `${state_dir}/exhausted` and **exit 2 with stderr feedback** telling the implementer: "Already exhausted (round=$round, max=$max_rounds). Do not retry TaskUpdate(status:'completed'). SendMessage NEEDS_HUMAN T### to the lead and go idle." Do not invoke review-gate. The `TaskUpdate` will not go through; the task stays `in_progress`; the lead's classifier sees `exhausted` + `in_progress` + (eventually) a NEEDS_HUMAN message.

   **Round-counter semantics (locked).** `max_rounds` = the number of review cycles permitted. The implementer gets up to `max_rounds` reviewed `TaskUpdate(completed)` attempts. All boundary checks compare the value of `round` **as read from disk, before any increment in this hook invocation**. With `max_rounds=3`, the sequence is: TaskUpdate #1 → round=0 read, review fires; TaskUpdate #2 → round=1 read, review fires; TaskUpdate #3 → round=2 read, review fires (final-round warning emitted in stderr); TaskUpdate #4 → round=3 read, short-circuits with `exhausted` marker + exit 2 (no review). The implementer is required by the prompt rule to send NEEDS_HUMAN and go idle rather than attempt #4 in the first place; the short-circuit is defense in depth.
...
5. **Derive commit range and run trailer guards.** With `baseline_sha` from the state file and the `Cerberus-Task: T###` trailer enforced on every implementer commit, use git's first-class trailer parser (avoids `--grep` regex-anchoring portability issues across BRE/ERE/PCRE — see `git interpret-trailers` and the `%(trailers:...)` pretty format):
   ...
   If `commits_for_task` is empty: **increment `round` in `state.json` first** (so a malformed-commit loop still hits the `round >= max_rounds` short-circuit in step 4), then **exit 2 with stderr** telling the implementer: `"No commits found tagged 'Cerberus-Task: ${task_id}' since baseline ${baseline_sha}. Make at least one commit with the required trailer (Cerberus-Task: ${task_id}) before retrying TaskUpdate(status:'completed'). Round <r> of <max_rounds>."`.
   ...
   **Untagged-commit guard (range-contract enforcement).** After the existence check passes, compute the inverse set: commits in `${commit_range}` that do **not** carry this task's trailer.
   ...
   ```
   If non-empty: increment `round` first, then exit 2 with stderr telling the implementer to amend / interactive-rebase / squash so every commit in `baseline..HEAD` carries the trailer, then retry `TaskUpdate(completed)`. Same rationale as the original SubagentStop design: step 7 sends the full range to `spawn-code-review`, so any untagged commit would otherwise be reviewed as in-scope, smuggling unrelated changes through the gate.

6. **Run the confirmed project verification gate before review.** Read `verify_script_path` from `state.json`; fail closed as `INFRA-FAILURE` if it is missing or the file does not exist. The lead writes this script only after user confirmation in Phase 1.5, so missing state means the run was started incorrectly.
...
6. **`TaskCompleted` input fields are partially undocumented.** Per https://code.claude.com/docs/en/hooks, the event exists and has no matcher and supports `exit 2` block-with-stderr-feedback, but the docs do not enumerate every input JSON field for `TaskCompleted` (only the common fields like `session_id`, `transcript_path`, `cwd` plus the listed `task_id`, `task_name`). The hook's defensive design — subject-prefix filter (which carries `<team_hash>`) + state-file lookup keyed by the parsed `T###` — does not depend on `task_metadata` being passed; metadata-based discovery of `cerberus_team_hash` and `cerberus_task_id` is the recommended primary path, with subject-prefix parsing as the fallback when metadata is not exposed at runtime. Re-confirm during implementation by inspecting actual hook input from a smoke run; if metadata is reliably exposed, prefer it over the subject-prefix fallback.
   - Codex's prior review suggested `TaskCompleted` may also fire on teammate idle with in-progress tasks; we re-checked the docs and that is `TeammateIdle`'s job, not `TaskCompleted`'s. If a future Claude Code version changes this contract (e.g., adds an idle-firing trigger to `TaskCompleted`), the design's escalation path may need to add a "don't double-fire on idle" guard. Watch the upstream docs.
   - The hook is now idempotent on prior PASS (step 3.5: if both `${state_dir}/verified_pass` and `${state_dir}/reviewed_pass` exist and `verified_head`/`reviewed_head` both equal current `HEAD`, exit 0 immediately). This means the design is robust to TaskCompleted firing more than once for the same task without rerunning verification or burning review rounds, while still rejecting stale markers after `HEAD` changes.
...
3. **End-to-end smoke**:
   ...
   - Lead discovers a smoke verification gate (for example `python -m pytest` if pytest exists, or an explicit no-op if this tiny repo has no test runner), presents it to the user, and does not call `TeamCreate` until the user confirms.
   - After confirmation, lead creates a team `cerberus-impl-<hash>`, writes `${TMPDIR:-/tmp}/cerberus-task-completed-hook/<hash>/verify.sh`, populates the TaskList with two `[CERBERUS-IMPL/<hash>]`-prefixed tasks, writes per-task state file at `${TMPDIR:-/tmp}/cerberus-task-completed-hook/<hash>/T001/state.json`, then spawns T001's implementer teammate.
   - Implementer commits to current branch with `Cerberus-Task: T001` trailer, calls `TaskUpdate(status: "completed")`, `TaskCompleted` hook fires, runs the confirmed verification script from the repo root, review-gate spawns reviewers, consensus reaches PASS, hook writes `verified_pass`, `verified_head`, `reviewed_pass`, and `reviewed_head`, exits 0, TaskUpdate succeeds, teammate goes idle.
...
4. **Verify-epic wiring** — same smoke setup but include real acceptance criteria in `smoke-plan.md` and run without `--skip-verify`. Expect: after both tasks pass, lead invokes `spawn-epic-verify` against `smoke-plan.md`, polls for verdict, includes the verdict in the final report.
5. **Failure modes** — exercise each of the five outcome categories defined in Phase 2 of `run-team.md`:
   - **needs-human**: author a deliberately broken task ("fix bug X that doesn't exist"). Confirm: implementer hits final-round FAIL, hook's stderr warns "FINAL REVIEWED ROUND", implementer respects the rule and SendMessages NEEDS_HUMAN to the lead, does NOT retry TaskUpdate, goes idle. Lead receives the message + sees task is `in_progress` + sees `exhausted` marker → classifies `needs-human` → reports + recovery instructions; no further tasks scheduled.
   ...
   - **abandoned**: hand-spawn an implementer with a prompt that immediately goes idle without making any commits or sending any messages. Confirm: lead receives TeammateIdle, sees task `in_progress` + no `exhausted` marker + no `last_error` marker + no `NEEDS_HUMAN` message, classifies as `abandoned`, stops scheduling, final report identifies the model-behavior failure (distinct from `needs-human`).
