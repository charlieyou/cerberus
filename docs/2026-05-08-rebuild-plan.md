...
| **D48 — OQ-Plan-3 resolved: Codex hook contract validated against Codex CLI 0.130.0** | Phase E validated the Codex hook contract used by v2 on 2026-05-09: Codex lifecycle hook payloads are delivered on stdin, Cerberus accepts the observed fields `session_id`, `transcript_path`, `project_key`, `transcript`, `cwd`, `workspace_root`, `prompt`, and `stop_reason`, and plugin-bundled hooks resolve their installed root through `${PLUGIN_ROOT}` with `CERBERUS_ROOT` as the explicit override. The hook manifest therefore invokes the Go binary as `bin/cerberus hook codex-session-start`, `bin/cerberus hook codex-prompt-submit`, and `bin/cerberus hook codex-stop`; it does not depend on legacy `bin/codex-session-init` or `bin/codex-stop-hook` scripts. | Installed CLI check: `codex --version` returned `codex-cli 0.130.0`. Implementation evidence: `internal/cli/hook.go` reads `os.Stdin`; `internal/hook/codex.go` decodes the accepted payload fields; `hooks/codex-hooks.json` uses `${CERBERUS_ROOT:-${PLUGIN_ROOT:-}}` and fixed Go hook subcommands; `tests/fixtures/codex/hook-payload-*.json`, `internal/hook/codex_test.go`, `tests/integration/codex_hook_skeleton_test.go`, and `tests/integration/codex_session_init_test.go` exercise the stdin payload contract. | Risk: Codex hook payloads remain host-owned and may add fields. The decoder ignores unknown fields, so additive payload changes are safe; removing `session_id` or transcript/root fields would require a new Decision-Log row and fixture update. |
...
- **OQ-Plan-3 — Codex hook payload/root contract.** **Resolved by D48.** Codex CLI 0.130.0 validation confirmed stdin payload handling and plugin-root behavior for the v2 Go hook manifest.
